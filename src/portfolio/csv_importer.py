...
class CSVImporter:
    ...
    def parse_csv(
        self, csv_content: str, format_type: Optional[str] = None
    ) -> ImportResult:
        """
        ...
        try:
            ...
            return ImportResult(
                0, 1, [], [{"row": 0, "error": f"Failed to parse CSV: {e}"}]
            )
        ...
        if format_type is None or format_type not in self.FORMATS:
            return ImportResult(
                ...
                [
                    {
                        "row": 0,
                        "error": "Unknown CSV format. Expected columns: date, symbol, type, quantity, price",
                    }
                ],
            )
