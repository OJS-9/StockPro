...
> **STATUS: MITIGATED** — `quality_gate_node` added to graph (March 2026). Routes `specialized_node → quality_gate_node → (synthesis_node | storage_node)`. Filters errored subjects, aborts synthesis if >50% failed, sets `is_partial_report` and `completeness` metadata on stored reports.

**Remaining risk:** Quality bar is binary (error vs. no error). Short but non-erroring outputs still pass. No minimum length or relevance check yet.
...
> **STATUS: PARTIALLY MITIGATED** — Two controls added (March 2026):
> 1. **`MAX_RESEARCH_SUBJECTS` env cap** — `_fan_out` slices `plan.selected_subject_ids[:_MAX_SUBJECTS]` (default 8, configurable via env var). Hard ceiling regardless of planner output.
> 2. **User subject selection popup** — Step 1 of the popup lists all eligible subjects (priority-1 pre-checked). Selected IDs flow through `start_generation → generate_report → run_research → ResearchState.user_selected_subjects → planner_node`, constraining the eligible list before the LLM call.

**Remaining risk:** No per-request token budget tracking. Cost is bounded by subject count but not by per-subject turn/token usage.

**Remaining improvement:**
- Track cumulative token usage via LangSmith run metadata.
- Add a conditional edge that aborts remaining `Send()` calls if spend exceeds a per-run budget.
...
|----------|------|--------|------------|------------|
| **P0** | #1 — No graph checkpointing | High — lost work/cost | Medium | Low (one-line change) | ~~Partial~~ — MemorySaver added; durable backend still needed |
| **P0** | #2 — Silent failure propagation | High — bad reports | Medium | Low | **MITIGATED** — quality_gate_node |
| **P0** | #7 — Synthesis truncation undetected | High — incomplete reports | High | Low | Open |
| **P1** | #3 — Unbounded token cost | High — cost overrun | Medium | Medium | **Implemented (best-effort preflight)** — subject cap + user selection + USD budget-to-depth throttling (estimated) |
| **P1** | #4 — Planner fallback runs all subjects | High — cost spike | Low | Low | Open |
| **P1** | #6 — No quality validation | Medium — bad synthesis | Medium | Medium | Open |
| **P1** | #8 — Agent recreated every turn | Medium — latency/waste | High | Low | Open |
...
> **STATUS: PARTIALLY MITIGATED** — `MemorySaver` checkpointer added (March 2026). In-memory only; lost on process restart.

**Where:** `src/research_graph.py`
...
**Remaining risk:** `MemorySaver` persists state in-process RAM only. A process restart (crash, deploy, timeout) still loses all in-flight work. For production durability, replace with `SqliteSaver` or `PostgresSaver`.

**Improvement:** Swap `MemorySaver` for a durable backend tied to the session `thread_id`.
...
> Non-blocking improvements. Check off as completed.

### P0 — Completed (2026-03-14)

- [x] #1 — MemorySaver checkpointer attached to `research_graph`; `run_research` passes a unique UUID `thread_id` per invocation. Files: `src/research_graph.py`.
- [x] #2 — `quality_gate_node` added between fan-in and synthesis: filters errored subjects, aborts with error report if >50% fail, passes `failed_subjects` + `is_partial_report` to synthesis. Files: `src/research_graph.py`, `src/agents/synthesis_node.py`.
- [x] #7 — Synthesis truncation detection: `END_OF_REPORT` marker check + length heuristic (`_is_truncated`); retry once with continuation prompt; flags as `[INCOMPLETE REPORT]` if retry also fails. `storage_node` writes `completeness: complete|partial` + `failed_subjects` to report metadata. Files: `src/agents/synthesis_node.py`, `src/research_graph.py`.

> Full plan + code snippets: `docs/plans/2026-03-14-p0-fixes-plan.md`. Verified by `test_research_graph_checkpointer.py` — 57/57 tests pass.

### P1 — High Priority

- [x] #3 — Unbounded token cost from parallel agents — **Implemented (best-effort preflight)** — adds per-run USD spend budget estimation and converts it into effective specialized depth caps (`effective_max_turns`, `effective_max_output_tokens`) applied in `research_graph._fan_out` and enforced in `agents/specialized_node.py`. Budget + effective settings are persisted into `reports.metadata` (JSON-safe). Files: `src/spend_budget.py`, `src/research_graph.py`, `src/agents/specialized_node.py`, `src/orchestrator_graph.py`, `src/app.py`, `templates/index.html`.
- [ ] #4 — Planner fallback silently runs all subjects (`src/agents/planner_node.py` `_fallback_plan()`) — cap fallback to top 3–4 priority subjects instead of all eligible; retry planner LLM once before falling back
- [ ] #6 — No validation of research output quality (`src/agents/specialized_node.py` lines 171–179) — add minimum output length check (<200 chars = failure), parse source URLs from text into structured `sources` field
- [ ] #8 — Orchestrator recreates ReAct agent on every turn (`src/orchestrator_graph.py` `_get_agent_response()`) — compile LLM and agent once in `__init__`, reuse across turns

### P2 — Structural

- [ ] #5 — Per-node client recreation not actually cached (`src/agents/specialized_node.py` `_get_clients()`) — add module-level `_mcp_client` / `_nimble_client` globals with initialization guard
- [ ] #9 — Race condition on shared mutable OrchestratorSession state (`src/orchestrator_graph.py` lines 32–38) — return results through graph state instead of mutating session object during report generation, or add a lock
- [ ] #12 — Fragile `recursion_limit` tied to max turns via arbitrary multiplier (`src/agents/specialized_node.py` line 153) — increase to `SPECIALIZED_MAX_TURNS * 3` or catch `GraphRecursionError` explicitly for graceful termination
- [ ] #13 — Full state sent to every parallel node via `Send()` (`src/research_graph.py` `_fan_out()`) — only pass fields each specialized node reads: `ticker`, `trade_type`, `plan`, `subject_id`, `emitter`

### P3 — Maintenance

- [ ] #10 — Duplicate rate limit retry logic (`src/agents/specialized_node.py` lines 88–93, 140–187) — extract `_is_rate_limit_error` and retry loop into shared `src/agent_retry.py` utility, or replace with `llm.with_retry()`
- [ ] #11 — MCP tool result truncation loses data silently (`src/langchain_tools.py` lines 33–37) — append `_{key}_note` truncation marker to result dict when clipping arrays
