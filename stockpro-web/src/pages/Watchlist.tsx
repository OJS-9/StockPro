...
function ItemMenu({ item, watchlistId, onClose }: { item: any; watchlistId: string; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', right: 0, top: 36, zIndex: 50, background: '#1c1917', border: '1px solid #292524', borderRadius: 10, overflow: 'hidden', minWidth: 180, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
      <button
        onClick={() => pinMutation.mutate()}
        style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', background: 'transparent', border: 'none', color: item.is_pinned ? '#22c55e' : '#fafaf9', fontSize: 13, cursor: 'pointer', textAlign: 'left' }}
      >
        <Icon name={item.is_pinned ? 'push_pin' : 'push_pin'} size={15} />
        {item.is_pinned ? 'Remove from home' : 'Pin to home page'}
      </button>
    ...
    <tr>
      <td colSpan={4} style={{ padding: '10px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={onDone} style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid #292524', background: 'transparent', color: '#a8a29e', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
        </div>
      </td>
    </tr>
  )
    ...
    <div style={{ background: '#0c0a09', minHeight: '100vh', color: '#fafaf9' }}>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 48px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            <div style={{ fontFamily: 'Nunito, sans-serif', fontSize: 26, fontWeight: 600, letterSpacing: '-0.02em', marginBottom: 4 }}>Watchlists</div>
            <div style={{ fontSize: 13, color: '#a8a29e' }}>{watchlists.length} list{watchlists.length !== 1 ? 's' : ''} &nbsp;&middot;&nbsp; Updated just now</div>
          </div>
        </div>
            ...
            <div key={wl.id} style={{ background: '#1c1917', border: '1px solid #292524', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #292524' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#a8a29e', marginTop: 2 }}>{wl.items?.length || 0} symbols</div>
                </div>
                <button
                ...
                >
                  <Icon name="add" size={14} /> Add symbol
                </button>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Symbol', 'Price', 'Change', 'Home', ''].map(h => (
                      <th key={h} style={{ fontSize: 10.5, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#a8a29e', textAlign: h === 'Symbol' ? 'left' : 'right', padding: '10px 20px', borderBottom: '1px solid #292524' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  ...
                  {(wl.items || []).map((item: any) => (
                    <tr key={item.symbol}>
                      ...
                      <td style={{ padding: '14px 20px', borderBottom: '1px solid rgba(41,37,36,0.5)', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontSize: 13.5, color: '#fafaf9' }}>
                        {item.price != null ? fmt(item.price) : '—'}
                      </td>
                      <td style={{ padding: '14px 20px', borderBottom: '1px solid rgba(41,37,36,0.5)', textAlign: 'right' }}>
                      ...
                      <td style={{ padding: '14px 20px', borderBottom: '1px solid rgba(41,37,36,0.5)', textAlign: 'right' }}>
                        {item.is_pinned ? (
                          <span style={{ fontSize: 11, color: '#22c55e', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <Icon name="push_pin" size={13} /> Pinned
                          </span>
                        ) : (
                      ...
                      <td style={{ padding: '14px 20px', borderBottom: '1px solid rgba(41,37,36,0.5)', textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center' }}>
                          <Link to={`/research?ticker=${item.symbol}`} style={{ width: 28, height: 28, borderRadius: 7, border: '1px solid #292524', background: 'transparent', color: '#a8a29e', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }} title="Research">
